...
$sql = "
	...
	WHERE 
		a.jobid=c.jobid and b.id=c.candidateid and c.status='Awaiting Feedback' and e.memberid=a.memberid and d.id=e.companyid  and f.companyid='$cid' and c.recruiterid=f.memberid and submitdate>=CURDATE()-INTERVAL 6 MONTH $employerres $jobtitleres $candres
	ORDER BY 
		submitdate desc
		"; 
...
/**if(isset($_POST['jobtitle']) && ($_POST['jobtitle']!='')  && isset($_POST['employer']) && ($_POST['employer']!='')){
	...
	WHERE 
		a.jobid=c.jobid and b.id=c.candidateid and c.status='Awaiting Feedback' and a.jobtitle='$jobtitle' and e.memberid=a.memberid and 
...
elseif(isset($_POST['jobtitle']) && ($_POST['jobtitle']=='')  && isset($_POST['employer']) && ($_POST['employer']!='')){
	...
	FROM 
		submitted_candidates c,	jobs a,	candidates b, companyprofile d, members e 
	WHERE 
		a.jobid=c.jobid and b.id=c.candidateid and c.status='Awaiting Feedback' and e.memberid=a.memberid and 
		d.id=e.companyid and d.name = '$employer' and c.recruiterid='$mid' 
	ORDER BY 
		submitdate desc
		"; 
...
$candidatemsg="<p style='
            font-size: 14px;
            color: #a02121;
        '>No pending feedbacks. Submit Candidates to Engaged Jobs.</p>";
?>
<!doctype html>
<html>
<head>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1">
     <title>Awaiting Feedback | Recruiters Hub</title>
...
</a> <a class="btn btn-primary" href="https://recruitinghub.com/RighttoRepresent.docx" target="_blank">
    <i class="fa fa"></i> Download Righ to Represent
</a> <a class="btn btn-primary" href="https://recruitinghub.com/Zero Tolerance Policy.pdf" target="_blank">
    <i class="fa fa"></i> Zero Tolerance Policy
</a>
<a class="btn btn-primary" href="awaiting-feedback-download<?php if($param)
 echo'?'.$param; ?>">
   <i class="fa fa-file-excel-o" ></i> Download in Excel
</a>
      <div class="panel panel-default">
      <header class="panel-heading wht-bg ui-sortable-handle">
    <h3 class="panel-title"> Awaiting feedback -> Win client's duplicate issue by taking RTR consent from Candidate. </h3> <!--<span class="label label-primary">Agencies have 6 months ownership over submitted candidates if not duplicated by Client - Keep in good contact with your candidates</span>
...
<div class="table-responsive">
 ...
 <form method="get" action="awaiting-feedback">
       <div class="form-group">
        ...
        <h3>Total Awaiting Feedback:<?php echo $total; ?> (displaying last 6 months submissions)</h3>
        <div class="text-center">
 <p style = 'font-size: 14px;
            color: #a02121;'><b>No feedback yet? Click Open the CV and click "<b>ADD NOTES</b>" to follow up with Client (Please be polite to clients when chasing for feedback)</b> </p>
</div>
  ...
  <small>Submitted:'.$row['submitdate'].' / '.$row['id'].'</small> | '.$row['jobtype'].' <br><small>Candidate Status:</small> <span class="d-inline-block" tabindex="0" data-toggle="tooltip" title="STATUS OF CANDIDATE SUBMITTED BY YOU"><i class="fa fa-question-circle"></i>
  ...
  <td> <a class="title" href="/recruiter/awaiting-feedback?jobtitle='.urlencode($row['jobtitle']).'&candidate=&employer=&status=&submit=Search" >'.$row['jobtitle'].'</a><br><b>'.$row['name'].'</b> / <small>'.$row['jobid'].'</small> </td>';
  ...
  <td class="text-right">
	...
	<li><a title="Delete Submission" class="confirmation" href="delete-submission?id='.$row['sid'].'")>Delete Submission</a></li>
