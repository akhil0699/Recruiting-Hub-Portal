...
$content="You have no closed or inactive or dis-engaged jobs in last 6 months.";
...
<head>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1">
     <title>Dis-Engaged Roles | Recruiters Hub</title>
...
<h4>Vendor Section (Business)</h4>
   ...
   <li><a href="becomefranchise" role="tab" > <i class="fa fa-database"></i>Become our Franchise <span class="label label-primary">New</span></a> </li>
    ...
    <li><a href="disengaged" role="tab"><i class="fa fa-scissors"></i>CLOSED / DISENGAGED</a>    </li>
...
<div class="account-navi-content">
  ...
  <div class="row">
    ...
    <h3 class="panel-title"> Dis-Engaged/Inactive/Closed Jobs <span class="d-inline-block" tabindex="0" data-toggle="tooltip" title="You are viewing the jobs that are either Closed/made Inactive by Employer/Disengaged by you. Jobs that are disengaged by you cannot be undone. You need to use the live chat to let us know the job id that you want to be re-engaged. Otherwise there is no Action Required from you"><i class="fa fa-question-circle"></i>
    </span></h3>
  </header>
        <div class="panel-body">
          ...
          <div class="row">
            <div class="col-md-12 table-responsive">
             <?php if(!$content){ ?>
             <div class="col-sm-12">
     <form method="get" action="disengaged">
       ...
       <!-- <label>Search Employer</label><br><br> -->
        ...
        <h3>Total INACTIVE/DISENGAGED Roles:<?php echo $total; ?> (Displaying Last 6 Months Data) </h3>
...
$checkrpo=$row['rpo'];
  ...
  <td> <a class="title" href="#" data-toggle="modal" data-target="#engage-modal" id='.$row['jobid'].'><b>'.$row['jobtitle'].'</b></a> <br>';	
	...
	echo '
	<small>'.$row['joblocation'].'</small> / <small>'.$row['jobtype'].'</small> / <small>Posted: '.$row['postdate'].'</small> /  <small>Updated: <b>'.$row['updatedate'].'</b></small> /<small>Job ID: '.$row['jobid'].'</small> </td>
	
	<td>';
	
    if(!(in_array($row['requeststatus'],array("Requested","Rejected"))))
  	echo '<a class="title engage-trigger" href="#" data-toggle="modal" data-target="#employer-modal" id='.$row['empid'].' >'.$row['employer'].'</a>  <br> </td>
...
echo '<td><b>'.$row['jobtype'].'</b></td>';
   
   
	if($adminrights)
	 echo '<td>'.$row['subuser'].'</td>';
	
	echo '<td>'.$row['applicants'].'/'.$row['cvlimit'].'</td>';echo'
	 <td>'.$row['agencycount'].'/'.$row['agencylimit'].' </td>
