...
$jobtitleres="and jobtitle like '%$jobtitle%'";
}

}
$jobres = mysqli_query($link,"SELECT * FROM jobs WHERE status='Hold' and memberid='$mid' $jobtitleres order by postdate desc"); 
//$jobres= mysqli_query($link,"SELECT a.jobtitle as jobtitle,a.jobid,a.joblocation,a.postdate, b.fname as candidatename,c.candidateid as candidateid,c.status as candidatestatus,e.name as recruiter,a.fee FROM submitted_candidates c,jobs a,candidates b,members d,companyprofile e WHERE a.memberid='$mid' and a.jobid=c.jobid and b.id=c.candidateid and c.status='Hold' and c.recruiterid= d.memberid and d.companyid=e.id  order by submitdate desc");
...
$content="So far you have no inactive jobs.";
...
<head>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1">
     <title>Inactive | Employers Hub</title>
...
<div class="account-navi-content">
  ...
  <div class="row">
    ...
    <h3 class="panel-title"> List of Inactive Jobs </h3>
  </header>
  
  <div class="panel-body">
        <div class="text-left">
 <a>Go to -> <a href="submit-a-job" id="one">POST NEW JOB</a></a> -> <a href="jobs" id="two">ACTIVE</a></a> -> <a href="inactive" id="three"><strong>INACTIVE (HOLD)</strong></a> -> <a href="closed" id="four">CLOSED</a> -> <a href="filled" id="five">FILLED</a>  
</div>
        ...
        <div class="panel-body">
          ...
          <div class="row">
            <div class="col-md-12 table-responsive">
              <?php if(!$content){ ?>
              <div class="col-sm-12">
             <form method="GET" action="inactive" >
       <div class="form-group">
        
      
        <div class="col-sm-3">
        	<input name="jobtitle" class="form-control ui-autocomplete-input" id="jobtitle" placeholder="Search Inactive Job" autocomplete="off" type="text">
        </div>
   ...
   <small>'.$row['joblocation'].'</small> /  <small>'.$row['jobtype'].'</small> / <small>'.$row['jobid'].'</small> / <small>Posted: '.$row['postdate'].' | Updated:  '.$row['updatedate'].'</small> </td>
   
   <td><a title="Reopen" href="job-action?jobid='.$row['jobid'].'&a=reopen"><b>Refresh Job</b></a> </td>
    ...
    $('#engage-modal').on('show.bs.modal', function(e) {
        ...
        $.ajax({
            ...
            success: function(data) {
				...
				//$modal.find('#engage-modal-footer a').attr("href", "edit-a-job.php?id="+ essayId);
            }
        });
    });
